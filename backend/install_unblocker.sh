...
echo "请选择语言 | Please select a language | Vui lòng chọn ngôn ngữ"
...
echo -e "Vui lòng lưu ý rằng ngôn ngữ bạn chọn sẽ ảnh hưởng đến đầu ra của chương trình backend"
...
case $language in
  ...
  2)
    ...
    ;;
  3)
    echo "Quản lý Apple ID của bạn theo một cách mới, một chương trình phát hiện và mở khóa Apple ID tự động dựa trên câu hỏi bảo mật"
    echo "Địa chỉ dự án: github.com/pplulee/appleid_auto"
    echo "Nhóm thảo luận dự án Telegram: @appleunblocker"
    echo "==============================================================="
    ;;
  *)
...
case $language in
  ...
  3)
    ...
    read -e api_key
    echo "Bạn có muốn bật cập nhật tự động? (y/n)"
    read -e auto_update
    echo "Vui lòng nhập chu kỳ đồng bộ hóa nhiệm vụ (đơn vị: phút, mặc định 15)"
    read -e sync_time
    if [ "$sync_time" = "" ]; then
        sync_time=15
    fi
    echo "Bạn có muốn triển khai bộ chứa Docker Selenium? (y/n)"
    read -e run_webdriver
    ;;
  *)
    echo "输入错误，已退出 | Input error, exit | Lỗi đầu vào, thoát"
...
if [ "$run_webdriver" = "y" ]; then
    echo "开始部署Selenium Docker容器 | Start deploying Selenium Docker container | Bắt đầu triển khai bộ chứa Docker Selenium"
    echo "请输入Selenium运行端口（默认4444） | Please enter Selenium running port (default 4444) | Vui lòng nhập cổng chạy Selenium (mặc định 4444)"
    read -e webdriver_port
    if [ "$webdriver_port" = "" ]; then
        webdriver_port=4444
    fi
    echo "请输入Selenium最大会话数（默认10） | Please enter the maximum session number (default 10) | Vui lòng nhập số phiên tối đa (mặc định 10)"
    ...
    echo "Webdriver Docker容器部署完成 | Webdriver Docker container deployed | Bộ chứa Docker Webdriver triển khai hoàn tất"
fi
...
case $language in
  ...
  2)
    ...
    ;;
  3)
    echo "Cài đặt hoàn tất, bắt đầu bộ chứa"
    echo "Tên bộ chứa mặc định: appleauto"
    echo "Phương pháp vận hành:"
    echo "Dừng: docker stop appleauto"
    echo "Khởi động lại: docker restart appleauto"
